...
public class Host {
	...
	public boolean vmCreate(Vm vm) {
		...
		a = (CloudSim.clock()+ " MIPS " + getAvailableMips() + " required " + totalRequestedMips + " result " + (getVmScheduler().getAvailableMips() - totalRequestedMips)+ " App " + vm.getVmm() + " device ");
        if(vm.getHost() != null){
                a += vm.getHost().getDatacenter().getName();
        }
        else{
                a += ("null "+this.getDatacenter().getName());
        }
		printHost(a);
		a = (CloudSim.clock() + " Storage " + getStorage() + " VM size " + vm.getSize() + " result " + (getStorage() - vm.getSize()) + " App " + vm.getVmm() + " device ");
		if(vm.getHost() != null){
				a += (vm.getHost().getDatacenter().getName());
		}
		else{
		...
		if (getStorage() < vm.getSize()) {
			Log.printLine("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by storage");
			Log.printLine("VM size: "+vm.getSize()+" Host storage: "+ storage);
			printHost("FAIL TO ALLOCATE VM size: "+vm.getSize()+" Host storage: "+ storage);
			return false;
		}
		...
		if (!getRamProvisioner().allocateRamForVm(vm, vm.getCurrentRequestedRam())) {
			Log.printLine("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by RAM");
			Log.printLine("RAM available: "+ ramProvisioner.getAvailableRam()+ " RAM requested: "+vm.getRam());
			printHost("FAIL TO ALLOCATE RAM available: "+ ramProvisioner.getAvailableRam()+ " RAM requested: "+vm.getRam());
			return false;
		}
		...
		if (!getBwProvisioner().allocateBwForVm(vm, vm.getCurrentRequestedBw())) {
			Log.printLine("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by BW");
			Log.printLine("Host BW: "+getBwProvisioner().getAvailableBw()+ " VM BW: "+vm.getBw());
			getRamProvisioner().deallocateRamForVm(vm);
			printHost("FAIL TO ALLOCATE Host BW: "+getBwProvisioner().getAvailableBw()+ " VM BW: "+vm.getBw());
			return false;
		}
		...
		if (!getVmScheduler().allocatePesForVm(vm, vm.getCurrentRequestedMips())) {
			Log.printLine("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by MIPS");
			Log.printLine("Host MIPS: "+getMaxAvailableMips()+ " VM BW: "+vm.getMips());
			getRamProvisioner().deallocateRamForVm(vm);
			getBwProvisioner().deallocateBwForVm(vm);
			printHost("FAIL TO ALLOCATE Host MIPS: "+getMaxAvailableMips()+ " VM BW: "+vm.getMips());
			return false;
		}
		...
		getVmList().add(vm);
		vm.setHost(this);
		a = (CloudSim.clock()+ " FIM MIPS " + getAvailableMips() + " required " + totalRequestedMips + " result " + (getVmScheduler().getAvailableMips() - totalRequestedMips)+ " App " + vm.getVmm() + " device ");
        if(vm.getHost() != null){
                a += vm.getHost().getDatacenter().getName();
        }
        else{
	...
	public String toString() {
		return "Host [id=" + id + ", storage=" + storage + ", ramProvisioner="
				+ ramProvisioner + ", bwProvisioner=" + bwProvisioner
				+ ", vmScheduler=" + vmScheduler + ", vmList=" + vmList
				+ ", peList=" + peList + ", failed=" + failed
				+ ", vmsMigratingIn=" + vmsMigratingIn + ", datacenter="
				+ datacenter + "]";
	}
