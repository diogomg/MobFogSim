 ...
 * Electrical & Computer Engineering National University of Singapore August
 ...
 * {@link gridsim.net.NetPacket}, but it collects information like bandwidths,
 ...
 * particular type of NetPacket is experiencing, set the size and network class
 * of an InfoPacket to the same as the NetPacket, and send it to the same
...
public class InfoPacket implements Packet {
	 ...
	 *        size of the packet
	 * @param srcID
	 *        The ID of the entity that sends out this packet
	 * @param destID
	 *        The ID of the entity to which this packet is destined
	 * @param netServiceType
	 *        the class of traffic this packet belongs to
	 * @pre name != null
	 * @post $none
	 */
	public InfoPacket(String name, int packetID, long size, int srcID, int destID,
		int netServiceType) {
		this.name = name;
		packetId = packetID;
		srcId = srcID;
	 ...
	 * 
	 * @param id
	 *        the id of the hop that this InfoPacket is traversing
	 * @pre id > 0
	 * @post $none
	 */
	...
	/**
	 * This method should be called by routers and other entities when this
	 * InfoPacket reaches them along with the current simulation time.
	 * 
	 * @param time
	 *        current simulation time, use {@link gridsim.CloudSim#clock()} to
	 *        obtain this
	 * @pre time >= 0
	 * @post $none
	 */
	...
	/**
	 * This method should be called by routers and other entities when this
	 * InfoPacket is leaving them. It should also supply the current simulation
	 * time.
	 * 
	 * @param time
	 *        current simulation time, use {@link gridsim.CloudSim#clock()} to
	 ...
	 * 
	 * @param baudRate
	 *        the entity's baud rate in bits/s
	 * @pre baudRate > 0
	 * @post $none
	 */
	 ...
	 * 
	 * @param netServiceType
	 *        the packet's network service type
