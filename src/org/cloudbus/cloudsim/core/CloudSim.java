...
//		int m=0;
//		for(MobileDevice st:AppExemplo2.getSmartThings())
//			if(st.isStatus())
//				m++;
//		System.out.println("Size smartThings list: "+AppExemplo2.getSmartThings().size());
		...
		if (future.size() > 0) {
			...
			processEvent(first);
			future.remove(first);
//			if(first.getTag() == CloudSimTags.VM_DATACENTER_EVENT)
//				CloudSim.getEntity(first.getSource()).getFutureQueue().remove(first);
			fit = future.iterator();
			...
			while (trymore) {
				...
				if (next.eventTime() == first.eventTime()) {
					processEvent(next);
					toRemove.add(next);
//					if(next.getTag() == CloudSimTags.VM_DATACENTER_EVENT)
//						CloudSim.getEntity(next.getSource()).getFutureQueue().remove(next);
					trymore = fit.hasNext();
				} else {
					trymore = false;
				}
	 ...
	 * @return
	 */
	public static SimEvent sendDCEvent(int src, int dest, double delay, int tag, Object data) {
		if (delay < 0) {
			throw new IllegalArgumentException("Send delay can't be negative.");
		}
	...
	public static boolean cancelAll(int src, Predicate p) {
		...
		while (iter.hasNext()) {
			ev = iter.next();
			if (ev.getSource() == src && p.match(ev)) {
//				System.out.println("src: "+src+" ev: "+ev.toString()+
//						" type: " + ev.getType() + " ev time: "+ ev.eventTime() +
//						" sched by: " + ev.scheduledBy() + " data :" + ev.getData());
				iter.remove();
			}
		}
	...
	public static boolean cancelAll(FutureQueue sourceQueue) {
		countador++;
		SimEvent ev = null;
		int previousSize = future.size();
		Iterator<SimEvent> iter = sourceQueue.iterator();
		while (iter.hasNext()) {
			ev = iter.next();
//				System.out.println("src: "+ev.scheduledBy() +" ev: "+ev.toString()+ 
//					" type: " + ev.getType() + " ev time: "+ ev.eventTime() +
//					" sched by: " + ev.scheduledBy() + " data :" + ev.getData());
//			System.out.println(future.remove(ev));
			future.remove(ev);
			iter.remove();
		}
		return previousSize < future.size();
