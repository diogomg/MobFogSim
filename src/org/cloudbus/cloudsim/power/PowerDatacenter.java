 ...
 * centers. If you are using any algorithms, policies or workload included in
 * the power package please cite the following paper: Anton Beloglazov, and
...
public class PowerDatacenter extends Datacenter {
	 ...
	 *         the exception
	 */
	public PowerDatacenter() { // MobFogSim

	}

	public PowerDatacenter(String name) {// MobFogSim
		super(name);
	}
	...
	protected double updateCloudetProcessingWithoutSchedulingFutureEventsForce() {
		...
		for (PowerHost host : this.<PowerHost>getHostList()) {
			for (Vm vm : host.getCompletedVms()) {
				...
				getVmList().remove(vm);
				Log.printLine("VM #" + vm.getId() + " has been deallocated from host #"
					+ host.getId());
			}
		}
	...
	protected void processVmMigrate(SimEvent ev, boolean ack) {
		...
		SimEvent event = CloudSim.findFirstDeferred(getId(), new PredicateType(
			CloudSimTags.VM_MIGRATE));
