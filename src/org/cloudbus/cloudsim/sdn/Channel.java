...
/**
 * This class represents a channel for transmission of data between switches. It
 * controls sharing of available bandwidth. Relation between Transmission and
 ...
 * simultaneous package transmissions. This is logical channel. One physical
 * link (class Link) can hold more than one logical channels (class Channel).
...
public class Channel {
	...
	public Channel(int chId, int srcId, int dstId, List<Node> nodes, List<Link> links,
		double bandwidth) {
		this.chId = chId;
		this.srcId = srcId;
		this.dstId = dstId;
	...
	public double getAdjustedRequestedBandwidth() {
		...
		// Find the slowest link (low bw) among all links where this channel is
		// passing through
		for (int i = 0; i < nodes.size() - 1; i++) {
			Node from = nodes.get(i);
			Link link = links.get(i);
	...
	private double getAllocatedBandwidthPerTransmission() {
		// If this channel shares a link with another channel, this channel
		// might not get the full BW from link.
	 ...
	 * @return delay to next transmission completion or Double.POSITIVE_INFINITY
	 *         if there is no pending transmissions
	 */
	public boolean updatePackageProcessing() {
		...
		Log.printLine(CloudSim.clock() + ": Channel.updatePackageProcessing() (" + this.toString()
			+ "):Time spent:" + timeSpent +
			", BW/host:" + getAllocatedBandwidthPerTransmission() + ", Processed:"
			+ processedThisRound);
	...
	public double nextFinishTime() {
		...
		if (delay < NetworkOperatingSystem.getMinTimeBetweenNetworkEvents()) {
			...
			// ":Channel: delay is too short: "+ delay);
	...
	/**
	 * @return list of Packages whose transmission finished, or empty list if no
	 *         package arrived.
	 */
