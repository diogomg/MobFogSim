 ...
 * functions. It manages channels between switches, and assigns packages to
...
public abstract class NetworkOperatingSystem extends SimEntity {
	 ...
	 */
	protected abstract boolean deployApplication(List<Vm> vms, List<Middlebox> middleboxes,
		List<Arc> links);
	...
	private void sendInternalEvent() {
		...
		if (channelTable.size() != 0) {
			...
			Log.printLine(CloudSim.clock() + ": " + getName()
				+ ".sendInternalEvent(): next finish time: " + delay);
	...
	private double nextFinishTime() {
		...
		if (earliestEft == Double.POSITIVE_INFINITY) {
			throw new IllegalArgumentException(
				"NOS.nextFinishTime(): next finish time is infinite!");
		}
		return earliestEft;
	...
	private void processCompletePackages(List<Channel> channels) {
		for (Channel ch : channels) {
			...
			for (Transmission tr : ch.getArrivedPackages()) {
				...
				Log.printLine(CloudSim.clock() + ": " + getName() + ": Package completed: " + pkg
					+ ". Send to destination:" + dest);
	...
	private Channel findChannel(int from, int to, int channelId) {
		...
		if (channel == null) {
			// there is no channel for specific flow, find the default channel
			// for this link
	...
	private Channel createChannel(int src, int dst, int flowId, Node srcNode) {
		...
		if (flowId != -1 && lowestBw < reqBw) {
			...
			// Cannot make channel.
			Log.printLine(CloudSim.clock() + ": " + getName()
				+ ": Free bandwidth is less than required.(" + getKey(src, dst, flowId)
				+ "): ReqBW=" + reqBw + "/ Free=" + lowestBw);
		}
	...
	public int getHostAddressByVmId(int vmId) {
		Vm vm = findVm(vmId);
		if (vm == null) {
			Log.printLine(CloudSim.clock() + ": " + getName() + ": Cannot find VM with vmId = "
				+ vmId);
			return -1;
		}
		...
		if (sdnhost == null) {
			Log.printLine(CloudSim.clock() + ": " + getName()
				+ ": Cannot find SDN Host with vmId = " + vmId);
			return -1;
		}
	...
	protected void initPhysicalTopology() {
		...
		try {
			JSONObject doc = (JSONObject) JSONValue.parse(new FileReader(
				this.physicalTopologyFileName));
			...
			while (iter.hasNext()) {
				...
				for (int n = 0; n < nums; n++) {
					...
					if (nodeType.equalsIgnoreCase("vm")) {
						// VM
						Vm vm = new TimedVm(vmId, userId, mips, pes, ram, bw, size, "VMM",
							new CloudletSchedulerTimeShared(), starttime, endtime);
						...
						// Middle box
						Vm vm = new Vm(vmId, userId, mips, pes, ram, bw, size, "VMM",
