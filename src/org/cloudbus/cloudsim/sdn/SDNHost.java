...
/**
 * Extended class of Host to support SDN. Added function includes data
 * transmission after completion of Cloudlet compute processing.
 * 
 * @author Jungmin Son
...
public class SDNHost extends SimEntity implements Node {
	...
	/*
	 * public void addVm(Vm vm){ vms.put(vm.getId(), vm); host.vmCreate(vm); }
	 */
	...
	private void processPackage(Package data) {
		...
		if (dstVm != null) {// Try to deliver package to a hosted VM
			...
			System.out.println("Warning package sent to wrong host. Host ID=" + host.getId()
				+ " DST VM ID=" + vmId + ", SRC VM ID=" + data.getDestination());
		}
	}
	...
	public Node getVMRoute(int src, int dest, int flowId) {
		...
		if (route == null) {
			this.printVMRoute();
			System.err.println("SDNHost: ERROR: Cannot find route:" + src + "->" + dest
				+ ", flow =" + flowId);
		}
