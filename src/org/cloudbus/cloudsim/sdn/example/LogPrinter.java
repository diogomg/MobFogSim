...
public class LogPrinter {
	public static void printEnergyConsumption(List<Host> hostList, List<Switch> switchList,
		double finishTime) {
	...
	public static void printWorkloadList(List<Workload> wls) {
		...
		for (int i = 0; i < SDNBroker.appId; i++) {
			Log.printLine("App Id (" + i + "): " + appIdNum[i] + " requests, Start="
				+ appIdStartTime[i] + ", Finish=" + appIdFinishTime[i] + ", Rate="
				+ (double) appIdNum[i] / (appIdFinishTime[i] - appIdStartTime[i])
