...
public class DCNSFog {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (FogDevice device : fogDevices) {
				// names of all Smart Cameras start with 'm'
				if (device.getName().startsWith("m")) {
					// fixing 1 instance of the Motion Detector module to each Smart Camera
			...
			if (CLOUD) {
				...
				moduleMapping.addModuleToDevice("object_detector", "cloud", numOfAreas
					* numOfCamerasPerArea);
				// placing all instances of Object Tracker module in the Cloud
				moduleMapping.addModuleToDevice("object_tracker", "cloud", numOfAreas
					* numOfCamerasPerArea); 
			}
	...
	private static FogDevice addArea(String id, int userId, String appId, int parentId) {
		...
		for (int i = 0; i < numOfCamerasPerArea; i++) {
			...
			// adding a smart camera to the physical topology. Smart cameras have been modeled as fog devices as well.
	...
	private static FogDevice createFogDevice(String nodeName, long mips, int ram, long upBw,
		long downBw, int level, double ratePerMips, double busyPower, double idlePower) {
	...
	private static Application createApplication(String appId, int userId) {
		...
		application.addAppEdge("object_detector", "user_interface", 500, 2000, "DETECTED_OBJECT",
			Tuple.UP, AppEdge.MODULE);
		// adding edge from Object Detector to Object Tracker module carrying tuples of type OBJECT_LOCATION
		application.addAppEdge("object_detector", "object_tracker", 1000, 100, "OBJECT_LOCATION",
			Tuple.UP, AppEdge.MODULE);
		// adding edge from Object Tracker to PTZ CONTROL (actuator) carrying tuples of type PTZ_PARAMS
