...
public class VRGameFog {
	...
	public static void main(String[] args) {
		...
		try {
			...
			DeterministicDistribution d2 = new DeterministicDistribution(EEG_TRANSMISSION_TIME);
			System.out.println(d2.getNextValue());
			System.out.println(d2.getNextValue());
			System.out.println(d2.getNextValue());
			System.out.println(d2.getNextValue());
			...
			if (CLOUD) {
				...
				// fixing all instances of the Connector module to the Cloud
				moduleMapping.addModuleToDevice("connector", "cloud", numOfDepts
					* numOfMobilesPerDept); 
				// fixing all instances of the Concentration Calculator module to the Cloud
				moduleMapping.addModuleToDevice("concentration_calculator", "cloud", numOfDepts
					* numOfMobilesPerDept);
				...
				// if the mode of deployment is cloud-based
				 // fixing all instances of the Connector module to the Cloud rest of the modules will be placed by the Edge-ward placement policy
				moduleMapping.addModuleToDevice("connector", "cloud", numOfDepts
					* numOfMobilesPerDept);
			}
	...
	private static void createFogDevices(int userId, String appId) {
		...
		for (int i = 0; i < numOfDepts; i++) {
			//adding a fog device for every Gateway in physical topology. The parent of each gateway is the Proxy Server
	...
	private static FogDevice addGw(String id, int userId, String appId, int parentId) {
		...
		// latency of connection between gateways and proxy server is 4 ms
		dept.setUplinkLatency(4);
		for (int i = 0; i < numOfMobilesPerDept; i++) {
			String mobileId = id + "-" + i;
			// adding mobiles to the physical topology. Smartphones have been
			//modeled as fog devices as well.
			FogDevice mobile = addMobile(mobileId, userId, appId, dept.getId());
			// latency of connection between the smartphone and proxy server is 4 ms
	...
	private static FogDevice addMobile(String id, int userId, String appId, int parentId) {
		...
		// latency of connection between EEG sensors and the parent Smartphone is 6 ms
		eegSensor.setLatency(6.0);
		display.setGatewayDeviceId(mobile.getId());
		// latency of connection between Display actuator and the parent Smartphone is 1 ms
	...
	private static Application createApplication(String appId, int userId) {
		...
		// adding periodic edge (period=1000ms) from Concentration Calculator to Connector module carrying tuples of type PLAYER_GAME_STATE
		application.addAppEdge("concentration_calculator", "connector", 100, 1000, 1000,
			"PLAYER_GAME_STATE", Tuple.UP, AppEdge.MODULE);
		...
		// adding periodic edge (period=1000ms) from Connector to Client module carrying tuples of type GLOBAL_GAME_STATE
		...
		application.addAppEdge("client", "DISPLAY", 1000, 500, "SELF_STATE_UPDATE", Tuple.DOWN,
			AppEdge.ACTUATOR);
		// adding edge from Client module to Display (actuator) carrying tuples of type GLOBAL_STATE_UPDATE
		application.addAppEdge("client", "DISPLAY", 1000, 500, "GLOBAL_STATE_UPDATE", Tuple.DOWN,
			AppEdge.ACTUATOR);
		...
		application.addTupleMapping("client", "CONCENTRATION", "SELF_STATE_UPDATE",
		...
		application.addTupleMapping("concentration_calculator", "_SENSOR", "CONCENTRATION",
			new FractionalSelectivity(1.0));
		// 1.0 tuples of type GLOBAL_STATE_UPDATE are emitted by Client module per incoming tuple of type GLOBAL_GAME_STATE
