	...
	//	for(String deviceName : mapping.keySet()){
		...
		System.out.println("mapping: "+mapping);
			for(String moduleName : mapping.get(serverCloudlet.getName()).keySet()){
				...
				getCurrentCpuLoad().put(serverCloudlet.getId(), getCurrentCpuLoad().get(serverCloudlet.getId())+(module.getMips()*numModules));
				createModuleInstanceOnDevice(module, serverCloudlet);
				getModuleInstanceCountMap().get(serverCloudlet.getId()).put(moduleName, mapping.get(serverCloudlet.getName()).get(moduleName));
				return;
			}
	}
